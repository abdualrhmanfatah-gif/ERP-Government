...
**المشكلة**: النظام الحالي يرحّل عند الدفع فقط (`PaymentOrderExecuted`). لا يوجد قيد استحقاق عند اعتماد طلب الصرف.

**الحل**: أمر جديد `CreateAccrualEntryCommand` يُنشئ قيد استحقاق بحسابات يختارها المستخدم، مع ربطه بطلب الصرف. قيد السداد يُنشأ ديناميكياً عند تسجيل الدفع.
...
2. **لا نحتاج توسيع JournalEntryGenerator** — الأمر الجديد ينشئ القيد مباشرة
3. **الحسابات تختارها المستخدم** — لا PostingRule ثابتة للاستحقاق
