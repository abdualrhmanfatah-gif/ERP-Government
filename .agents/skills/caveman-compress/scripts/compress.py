...
# Bounds each individual model call so a stalled CLI (dropped network, an
...
MODEL_CALL_TIMEOUT_SECONDS = LOCK_WAIT_SECONDS // (MAX_RETRIES + 1)


# ---------- Model Calls ----------


def extract_text_parts(stdout: str) -> str:
    """Collect assistant text from an ``opencode run --format json`` stream."""
    texts: List[str] = []
...
def call_model(prompt: str) -> str:
    """Send a prompt to the locally configured opencode agent.

    Runs ``opencode run --format json`` with the prompt on **stdin** so long
    documents never hit Windows command-line length limits, then collects the
    assistant text parts from the JSON event stream. Authentication and model
    selection come from the user's opencode configuration; ``CAVEMAN_OPENCODE_MODEL``
    may override the model using ``provider/model`` format.
    """
    opencode_bin = shutil.which("opencode") or "opencode"
    command = [opencode_bin, "run", "--format", "json"]
    ...
    try:
        ...
        raise RuntimeError(
            "opencode CLI not found on PATH; install opencode (or fix PATH) so the "
            "caveman-compress skill can call it"
        )
    except subprocess.CalledProcessError as e:
        detail = (e.stderr or e.stdout or "").strip()[:2000]
        raise RuntimeError(f"opencode call failed:\n{detail}")
    except subprocess.TimeoutExpired:
        raise RuntimeError(
            f"opencode call timed out after {MODEL_CALL_TIMEOUT_SECONDS}s "
            "(stalled network, or an interactive auth prompt with no TTY to answer it)"
        )
    return strip_llm_wrapper(extract_text_parts(result.stdout))
...
def restore_code_blocks(text: str, blocks: List[Tuple[str, str]]) -> str:
    ...
    for marker, block in blocks:
        if restored.count(marker) != 1:
            raise ValueError(
                f"the model changed preserved code marker {marker}; refusing to write"
            )
    ...
    if CODE_MARKER_PREFIX in restored:
        raise ValueError("the model returned an unknown Caveman code-preservation marker")
    return restored
...
def _compress_file_locked(filepath: Path) -> bool:
    ...
    # Split YAML frontmatter off before compression. Models tend to strip or
    ...
    masked_compressed = call_model(build_compress_prompt(masked_body))
    try:
    ...
    if compressed_body is None or not compressed_body.strip():
        print("❌ Compression aborted: opencode returned an empty response.")
        print("   Original file is untouched (no backup created).")
        return False
    ...
    for attempt in range(MAX_RETRIES):
        ...
        if fixed is None or not fixed.strip():
            print("❌ Fix attempt aborted: opencode returned an empty response.")
