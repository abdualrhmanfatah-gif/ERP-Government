...
[Authorize(Policy = PermissionCodes.BudgetsSubmit)]
public record SubmitBudgetCommand(
    int Id,
    byte[] RowVersion) : IRequest<Result>;

public class SubmitBudgetCommandHandler(
    IApplicationDbContext context,
    IUser currentUser,
    IDocumentStatusLogger statusLogger) : IRequestHandler<SubmitBudgetCommand, Result>
{
    public async Task<Result> Handle(
        SubmitBudgetCommand request,
        CancellationToken cancellationToken)
    {
        ...
        if (entity.Status != BudgetStatus.Draft)
            return Result.Failure(["Only Draft budgets can be submitted."]);
        ...
        RecordApproval(context, entity, BudgetStatus.Draft, BudgetStatus.Submitted, currentUser.Id);
