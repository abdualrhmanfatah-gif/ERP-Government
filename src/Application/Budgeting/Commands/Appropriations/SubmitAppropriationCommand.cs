...
[Authorize(Policy = PermissionCodes.AppropriationsSubmit)]
public record SubmitAppropriationCommand(
    int Id,
    byte[] RowVersion) : IRequest<Result>;

public class SubmitAppropriationCommandHandler(
    IApplicationDbContext context,
    IUser currentUser,
    IDocumentStatusLogger statusLogger) : IRequestHandler<SubmitAppropriationCommand, Result>
{
    public async Task<Result> Handle(
        SubmitAppropriationCommand request,
        CancellationToken cancellationToken)
    {
        ...
        if (entity.Status != AppropriationStatus.Draft)
            return Result.Failure(["Only Draft appropriations can be submitted."]);
        ...
        RecordApproval(context, entity, AppropriationStatus.Draft, AppropriationStatus.PendingApproval, currentUser.Id);

        await statusLogger.LogAsync(
            "appropriations",
            entity.Id,
            AppropriationStatus.Draft.ToString(),
