...
import { Page, Button, Input, Select, Card, Combobox, Alert, Switch, Tabs, StatusBadge, EmptyState } from '@/components/ui';
import { Coins, BookOpen } from 'lucide-react';
import { useAssetGroupDetail, useUpdateAssetGroup, useAssetGroupsList } from '../hooks/useAssetGroups';
import { updateAssetGroupSchema, type UpdateAssetGroupInput } from '../shared/schemas';
...
export default function AssetGroupEditPage() {
  const navigate = useNavigate();
  const { id } = useParams<{ id: string }>();
  const groupId = Number(id);
  const [activeTab, setActiveTab] = useState('params');

  const { data: group, isLoading: groupLoading, error, refetch } = useAssetGroupDetail(groupId);
  ...
  } = useForm<UpdateAssetGroupInput>({
    resolver: zodResolver(updateAssetGroupSchema),
    values: group ? {
      name: group.name,
      description: group.description ?? null,
      parentAssetGroupId: group.parentAssetGroupId ?? null,
      assetCategory: group.assetCategory,
      isDepreciable: group.isDepreciable,
      depreciationMethod: group.depreciationMethod,
      depreciationRate: group.depreciationRate ?? null,
      defaultUsefulLifeYears: group.defaultUsefulLifeYears ?? null,
      residualValuePercentage: group.residualValuePercentage ?? null,
      assetAccountId: group.assetAccountId ?? null,
      accumulatedDepreciationAccountId: group.accumulatedDepreciationAccountId ?? null,
      depreciationExpenseAccountId: group.depreciationExpenseAccountId ?? null,
      disposalAccountId: group.disposalAccountId ?? null,
  ...
  async function onSubmit(data: UpdateAssetGroupInput) {
    try {
      ...
      navigate(`/assets/asset-groups/${groupId}`);
    } catch (err) {
      handleApiError(err, setError);
    }
  }

  const onInvalid = (formErrors: FieldErrors<UpdateAssetGroupInput>) => {
  ...
  return (
    <Page
      ...
      maxWidth="xl"
      onBack={() => navigate(`/assets/asset-groups/${groupId}`)}
    >
      <form onSubmit={handleSubmit(onSubmit, onInvalid)} className="flex flex-col gap-4" aria-label="نموذج مجموعة الأصول">
        ...
        <Card>
          ...
          <div className="flex flex-wrap items-end gap-3">
            <div className="w-36 shrink-0">
              <Input label="الكود" value={group.code} disabled readOnly dir="ltr" className="tabular-nums font-mono" />
            </div>
        ...
        <div className="flex justify-end gap-2">
          <Button variant="ghost" type="button" onClick={() => navigate(`/assets/asset-groups/${groupId}`)}>
            إلغاء
          </Button>
          <Button type="submit" variant="primary" loading={updateMutation.isPending || isSubmitting}>
            حفظ التعديلات
          </Button>
        </div>
      </form>
